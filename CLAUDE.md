...
- `native/AudioCapture/` — Swift helper. Captures mic (AVAudioEngine) + system audio (ScreenCaptureKit) into two WAVs. Speaks NDJSON on stdout; stops on `stop\n` via stdin or EOF.
...
- **Swift helper over Chromium's loopback capture.** Electron's `getDisplayMedia` loopback on macOS is unreliable, lossy (Opus), and has two unsynchronized clocks — fatal for merging. ScreenCaptureKit gives clean PCM and one process to reason about permissions.
...
- **macOS TCC (permissions):** helpers spawned by Electron are attributed to the dev `Electron.app` (`com.github.Electron`). Mic prompt works out of the box; system audio needs a one-time grant under Screen & System Audio Recording. From a sandboxed/CI shell there is no prompt — capture just fails with `-3801`.
