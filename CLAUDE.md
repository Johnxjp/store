...
- `src/main/` — services: `recorder` (Swift helper lifecycle), `transcribe` (ffmpeg + fluid-transcribe), `chunker` (pure PCM chunking for live transcription), `live` (during-meeting chunked transcription; any failure falls back to batch), `warm` (Ollama prefix-cache warming while recording), `merge` (pure timeline logic), `enhance` (prompt + Ollama), `pipeline` (orchestration + status), `db`, `config`, `paths`.
...
- **The mic WAV ends up to one tap buffer early** (issue #1, closed 2026-09-23, no action). The tap hands over whole 4096-frame buffers and `removeTap` drops the one in progress at Stop: up to 100 ms on the built-in mic, 256 ms on a 16 kHz Bluetooth headset. It was first misread as a start-anchor bias; hardware timestamps put both anchors within 5–20 ms of truth, so transcript timing is unaffected and only the tail after the Stop press is lost. The same buffer granularity bounds the pause boundary lag, and both end-of-file checks in `scripts/test-pause-capture.ts` allow 300 ms for it. Details: `docs/pause-and-resume-recording.md` § What the hardware actually delivers.
